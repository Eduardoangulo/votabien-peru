...
export function CalendarDatePicker({
  ...
  const years = centerCurrentYear
    ? (() => {
        const halfRange = Math.floor(yearsRange / 2);
        return Array.from(
          { length: yearsRange },
          (_, i) => today.getFullYear() - halfRange + i,
        );
      })()
    : Array.from(
        { length: yearsRange + 1 },
      ...
      <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
        ...
        {isPopoverOpen && (
          ...
          >
            <div className="flex">
              {numberOfMonths === 2 && (
                <div className="hidden md:flex flex-col gap-1 pr-2 text-left border-r border-foreground/10">
